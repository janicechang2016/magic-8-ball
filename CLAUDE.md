...
- **Masthead:** "my dumbass boyfriend magic 8 ball" in Shrikhand with hot-pink fill + deep-ink stroke. Small (12px) pink-pill kicker reads "send his dumbass to the stratosphere girl <3". (Both masthead + kicker are theme-swapped per side — see `THEMES`.)
...
- **`#side-link` placement is responsive:** desktop = a bookmark **tab on the right edge** (`position:fixed`); mobile (`@media (pointer: coarse)`) = a **dashed tab at the bottom** of the `#ui` stack (`order:9`, in-flow). Same element/behavior, two looks.
